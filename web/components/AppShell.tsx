...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className={collapsed ? "app-frame sidebar-collapsed" : "app-frame"}>
    <aside className="app-sidebar">
      ...
      <footer>
        {identity ? <>
          <div className="app-user" title={identity.email}>
            <i>{identity.name.slice(0, 1).toUpperCase()}</i>
            <span><b>{identity.name}</b><small>{identity.role}</small></span>
          </div>
          <button className="app-signout" onClick={() => { signOut(); setIdentity(null); router.push("/"); }}>Sign out</button>
    ...
    <div className="app-stage">
      <header className="app-topbar">
        <div className="switch-wrap"><button className="switch-button" onClick={() => setSwitching((open) => !open)}><small>Switch project</small><b>{current?.name || "All projects"}</b><span>⌄</span></button>{switching && <div className="switch-menu"><input autoFocus aria-label="Search projects" placeholder="Search projects…" value={projectQuery} onChange={(event) => setProjectQuery(event.target.value)} /><Link href="/projects">All projects</Link>{filtered.slice(0, 8).map((project) => <Link key={project.project_id} href={`${paths.project(project.project_id, project.name)}`}>{project.name}<small>{project.you_are_owner ? "Owner" : project.you_can_upload ? "Editor" : "Guest"}</small></Link>)}</div>}</div>
