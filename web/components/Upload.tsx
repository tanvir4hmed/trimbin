...
export default function Upload({ projectId, plan, canResolve = true, onFinished }: { projectId: number; plan: PlannedScene[]; canResolve?: boolean; onFinished?: () => void }) {
  ...
  useEffect(() => {
    ...
    setEvidenceUri(draft?.evidence_uri || selected.slate_uri || selected.slate_candidates?.[0] || "");
  }, [selected?.clip_id]);
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      if (cancelled) {
        setStage("add");
        setError("Upload cancelled. Files that already reached storage were not ingested or deleted.");
        return;
      }
  ...
  const reset = () => {
    setStage("add"); setFiles([]); setRows([]); setJobId(""); setStatus(null); setDecisions({}); setSelectedId(""); setTargetTake(0); setError("");
    window.localStorage.removeItem(storageKey);
    if (fileInput.current) fileInput.current.value = "";
  };

  if (!canResolve) return <section className="ingest-wizard ingest-authority"><header className="ingest-head"><div><p className="eyebrow">FOOTAGE INGEST</p><h1>Sign in with upload authority</h1><p>You can review this public project, but camera originals can only be added by its editors. A guest-owned project keeps full ingest authority for its owner.</p></div><span className="safety-lock">◈ No files have been selected or sent</span></header></section>;
  ...
  return <section className="ingest-wizard">
    ...
    {(stage === "verify" || stage === "ingest") && <div className="ingest-verify">
      <section className="ingest-table-panel"><header><div><h2>{items.length} clips</h2><p>{items.filter((item) => item.status === "Matched").length} matched · {items.filter((item) => item.status === "Needs review").length} need review · {items.filter((item) => item.status === "Duplicate").length} duplicate</p></div><div className="status-filter">Verify every row before commit</div></header><div className="ingest-table-wrap"><table className="ingest-table"><thead><tr><th>Clip</th><th>Duration</th><th>Camera</th><th>Detected assignment</th><th>Evidence confidence</th><th>Status</th></tr></thead><tbody>{items.map((item) => <tr key={item.clip_id} className={selected?.clip_id === item.clip_id ? "selected" : ""} onClick={() => setSelectedId(item.clip_id)}><td><div className="clip-cell">{item.slate_uri ? <img src={item.slate_uri} alt="Slate frame" /> : <span className="clip-thumb">▶</span>}<span><b>{item.filename || item.clip_id.slice(0, 8)}</b><small>Take {item.take_no || "—"}</small></span></div></td><td>{item.duration_s.toFixed(1)}s</td><td>{item.camera || <span className="dim">Not detected</span>}</td><td>{item.scene ? `Scene ${item.scene} / Shot ${item.shot || "—"} / Take ${item.take_no || "—"}` : "Unassigned"}</td><td><span className="confidence-meter"><i style={{ width: `${item.confidence * 100}%` }} /></span>{Math.round(item.confidence * 100)}%</td><td><span className={`ingest-status ${item.status.toLowerCase().replace(" ", "-")}`}>{item.status}</span></td></tr>)}</tbody></table></div></section>
      <aside className="ingest-inspector">{selected ? <><p className="eyebrow">SLATE EVIDENCE</p>{evidenceUri || selected.slate_uri ? <img className="inspector-slate" src={evidenceUri || selected.slate_uri} alt="Selected slate frame" /> : <div className="inspector-slate missing">No readable board frame</div>}{Boolean(selected.slate_candidates?.length) && <div className="slate-candidates">{selected.slate_candidates.map((uri) => <button key={uri} className={uri === evidenceUri ? "on" : ""} onClick={() => { setEvidenceUri(uri); const previous = decisions[selected.clip_id] || { action: "keep" as const, scene: selected.scene, shot: selected.shot, take: selected.take_no }; void choose(selected.clip_id, { ...previous, evidence_uri: uri }); }}><img src={uri} alt="Slate candidate" /></button>)}</div>}<h2>{selected.filename}</h2><div className="slate-read"><span>Scene <b>{selected.scene || "—"}</b></span><span>Shot <b>{selected.shot || "—"}</b></span><span>Take <b>{selected.take_no || "—"}</b></span><span>Camera <b>{selected.camera || "Not detected"}</b></span></div><p className="raw-read">{selected.slate_raw || "No slate text was confidently read."}</p>{selected.mismatch && <div className="mismatch-callout"><b>Assignment mismatch</b><p>{selected.mismatch}</p></div>}{selected.duplicate_of && <div className="mismatch-callout"><b>Duplicate evidence</b><p>Same bytes as clip {selected.duplicate_of.slice(0, 8)}. Kept; never auto-deleted.</p></div>}<div className="evidence-chips">{selected.slate_uri && <span>Slate frame</span>}{selected.slate_raw && <span>Slate read</span>}<span>Folder target</span><span>Container duration</span></div>
        {stage === "verify" && canResolve && <><div className="resolve-target"><label>Scene<select value={targetScene || selected.scene} onChange={(event) => { setTargetScene(Number(event.target.value)); setTargetShot(0); }}><option value={0}>Unassigned</option>{plan.map((item) => <option key={item.scene} value={item.scene}>{item.scene} · {item.heading}</option>)}</select></label><label>Shot<select value={targetShot || selected.shot} onChange={(event) => setTargetShot(Number(event.target.value))}><option value={0}>Choose shot</option>{(plan.find((item) => item.scene === (targetScene || selected.scene))?.shots ?? []).map((item) => <option key={item.shot} value={item.shot}>{item.slug || item.shot}</option>)}</select></label><label>Take<input type="number" min="0" value={targetTake || selected.take_no || ""} onChange={(event) => setTargetTake(Number(event.target.value))} /></label></div><label className="create-shot-toggle"><input type="checkbox" checked={createNew} onChange={(event) => setCreateNew(event.target.checked)} /> Create scene / shot if it is not in the plan</label><div className="resolve-buttons"><button className="primary" onClick={() => choose(selected.clip_id, createNew ? { action: "create", scene: targetScene || selected.scene, shot: targetShot || selected.shot, take: targetTake || selected.take_no, evidence_uri: evidenceUri } : { action: "move", scene: targetScene || selected.scene, shot: targetShot || selected.shot, take: targetTake || selected.take_no, evidence_uri: evidenceUri })}>Move / choose manually</button><button className="ghost" onClick={() => choose(selected.clip_id, { action: "keep", scene: selected.scene, shot: selected.shot, take: selected.take_no, evidence_uri: evidenceUri })}>Keep proposed match</button><button className="ghost" onClick={() => choose(selected.clip_id, { action: "unassign", take: selected.take_no, evidence_uri: evidenceUri })}>Leave unassigned</button></div>{decisions[selected.clip_id] && <p className="decision-ready">✓ Decision ready: {decisions[selected.clip_id].action}</p>}</>}
