...
import { cancelUpload, dismissUpload, pauseUpload, restoreUploadSnapshots, resumeUpload, subscribeUploads, uploadSnapshots } from "@/lib/upload";
...
export default function UploadTray() {
  ...
  return <aside className="global-upload-tray" aria-label="Upload progress">{batches.map((batch) => {
    ...
    return <section key={batch.id}><header><span><b>{batch.state === "done" ? "Upload complete" : batch.state === "interrupted" ? "Upload interrupted" : batch.state === "paused" ? "Upload paused" : batch.state === "cancelled" ? "Upload cancelled" : "Uploading footage"}</b><small>{batch.state === "interrupted" ? "Return to this project's ingest page and reselect the same files to resume" : `${batch.rows.filter((row) => row.state === "done").length}/${batch.rows.length} files · ${pct}%`}</small></span><div>{batch.state === "uploading" && <button onClick={() => pauseUpload(batch.id)}>Pause</button>}{batch.state === "paused" && <button onClick={() => resumeUpload(batch.id)}>Resume</button>}{!finished && batch.state !== "interrupted" && <button onClick={cancel}>Cancel</button>}{finished && <button onClick={() => dismissUpload(batch.id)}>Dismiss</button>}</div></header><div className="upload-tray-bar"><i style={{ width: `${pct}%` }} /></div></section>;
  })}</aside>;
}
