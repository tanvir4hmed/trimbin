...
export default function ReviewQueuePage() {
  ...
  return <main className="shell review-index">
    ...
    {queue.length ? <div className="review-queue">{queue.map((item, index) => <Link href={`${paths.shot(item.project_id, item.scene, item.shot, item.project_name)}`} key={`${item.project_id}-${item.scene}-${item.shot}`}><span className="queue-rank">{String(index + 1).padStart(2, "0")}</span><span><b>{item.project_name}</b><strong>Scene {item.scene} / {item.slug || `Shot ${item.shot}`}</strong><small>{item.takes} takes · {item.reason.replaceAll("_", " ")}{item.open_comments ? ` · ${item.open_comments} open notes` : ""}</small></span><span className={`queue-state ${item.state || "needs_review"}`}>{item.state?.replaceAll("_", " ") || "needs review"}</span><i>Open cockpit →</i></Link>)}</div> : shotsExist ? <div className="first-run"><h2>Nothing needs you</h2><p>Every shot is settled or already in somebody’s hands.</p><Link className="ghost" href="/home">Back home</Link></div>
      : projects.length ? <div className="first-run"><h2>No footage yet</h2><p>This queue fills itself as takes arrive. Upload a shoot day and the shots that need a person appear here.</p><Link className="primary" href={`${paths.ingest(projects[0].project_id, projects[0].name)}`}>Add footage</Link></div>
      : <div className="first-run"><h2>No projects yet</h2><p>Make a project, declare its scenes and shots, then upload the day&rsquo;s takes.</p><Link className="primary" href="/home">Make a project</Link></div>}
  </main>;
}
