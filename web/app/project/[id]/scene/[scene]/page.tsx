...
export default function SceneCoveragePage({ params }: { params: Promise<{ id: string; scene: string }> }) {
  ...
  const entries = useMemo(() => data?.timeline.flatMap((item) => item.entry ? [item.entry] : []) ?? [], [data]);
  ...
  const openEntry = (entry: StringoutEntry, at?: number) => {
    const found = entries.findIndex((candidate) => candidate.clip_id === entry.clip_id);
  ...
  return <main className="coverage-shell">
    ...
    <div className="coverage-grid">
      <section className="coverage-main">
        <div className="coverage-player"><Player ref={player} className="player" src={active?.proxy_uri ?? ""} poster={active?.sprite_uri} onReady={seekStart} onTimeUpdate={(time) => { if (active && time >= active.end_s - .04) advance(); }} onEnded={advance} /><div className="coverage-now"><span>{active?.slug ?? "No confirmed take"}</span>{active && <span>Take {active.take_no} · {clock(active.start_s)}–{clock(active.end_s)}</span>}</div></div>
        ...
        <div className="coverage-takes">{data.timeline.map((item) => item.entry ? <button key={item.shot} className={item.entry.clip_id === active?.clip_id ? "coverage-take on" : "coverage-take"} onClick={() => openEntry(item.entry!)}><img src={item.entry.sprite_uri} alt=""/><span><b>{item.slug}</b><small>Take {item.entry.take_no} · {clock(item.entry.duration_s)}</small></span></button> : <Link key={item.shot} className="coverage-take gap" href={`/project/${projectId}?scene=${sceneId}&shot=${item.shot}`}><span className="gap-icon">＋</span><span><b>{item.slug}</b><small>Choose a take</small></span></Link>)}</div>

        <section className="nle-context"><header><div><p className="eyebrow">READ-ONLY DECISION TIMELINE</p><h2>Scene {sceneId} coverage</h2></div><span>V1 / A1 · source ranges, not an edit</span></header><div className="nle-ruler"><span>00:00</span><span>{clock(data.duration_s * .25)}</span><span>{clock(data.duration_s * .5)}</span><span>{clock(data.duration_s * .75)}</span><span>{clock(data.duration_s)}</span></div>{(["V1","A1"] as const).map((track) => <div className="nle-track" key={track}><b>{track}</b><div>{data.timeline.map((item) => item.entry ? <button key={`${track}-${item.shot}`} className={`nle-block shot-${item.shot % 5}`} style={{ flexGrow: Math.max(2, item.entry.duration_s) }} onClick={() => openEntry(item.entry!)}><span>{item.slug} / Take {item.entry.take_no}</span>{track === "A1" && <i className="waveform"/>}</button> : <Link key={`${track}-${item.shot}`} href={`/project/${projectId}?scene=${sceneId}&shot=${item.shot}`} className="nle-gap"><span>GAP</span><small>No confirmed take</small></Link>)}</div></div>)}</section>
      </section>
