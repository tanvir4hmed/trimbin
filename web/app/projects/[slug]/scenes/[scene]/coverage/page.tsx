...
export default function SceneCoveragePage({
  ...
  const activeShotIndex = selectedShots.findIndex((item) =>
    (item.entries ?? (item.entry ? [item.entry] : [])).some((entry) =>
      entry.segment_id
        ? entry.segment_id === active?.segment_id
        : entry.clip_id === active?.clip_id &&
          entry.start_s === active?.start_s,
    ),
  );
  ...
  const openEntry = (entry: StringoutEntry, at?: number) => {
    const found = entries.findIndex((candidate) =>
      candidate.segment_id
        ? candidate.segment_id === entry.segment_id
        : candidate.clip_id === entry.clip_id &&
          candidate.start_s === entry.start_s,
    );
    if (found >= 0) setIndex(found);
    window.setTimeout(
      () => player.current?.seek(at ?? entry.start_s, true),
      found === index ? 0 : 120,
    );
    setPlaying(true);
  };
  const moveShot = (direction: number) => {
    const next =
      selectedShots[
        Math.max(
          0,
          Math.min(selectedShots.length - 1, activeShotIndex + direction),
        )
      ];
  ...
  if (error)
    return (
      <main className="scene-state">
        <p className="error">{error}</p>
        <Link href={`${paths.project(projectId)}`}>Back to project</Link>
      </main>
    );
  if (!data) return <main className="scene-state">Loading coverage…</main>;
  const sceneCode = data.entries[0]?.scene_code || sceneId;
    ...
    <main className="coverage-shell">
      <header className="coverage-head">
        ...
        <div className="coverage-stats">
          ...
          <a
            className="ghost small"
            href={api.markersUrl(projectId, sceneId, data.export_fps || 24)}
          >
            Markers
          </a>
        </div>
      ...
      <div className="coverage-grid">
        <section className="coverage-main">
          <div className="coverage-player">
            <Player
              ref={player}
              className="player"
              src={active?.proxy_uri ?? ""}
              poster={active?.sprite_uri}
              emptyLabel="No source ranges have been chosen for this scene yet. Open a shot and pick the parts of each take you want."
              onReady={seekStart}
              onTimeUpdate={(time) => {
                if (active && time >= active.end_s - 0.04) advance();
              }}
              onEnded={advance}
            />
          ...
          <div className="coverage-transport">
            <button
              className="ghost"
              onClick={() => moveShot(-1)}
            ...
            <button className="primary" onClick={toggle} disabled={!active}>
              {playing ? "Pause reel" : "Play coverage reel"}
            </button>
            <button
              className="ghost"
              onClick={() => moveShot(1)}
              disabled={
                activeShotIndex < 0 ||
                activeShotIndex >= selectedShots.length - 1
              }
            >
              Next shot →
            </button>
        ...
        <aside className="coverage-inspector">
          <nav>
            <button
            ...
            >
              Notes <span>{data.notes.length}</span>
            </button>
            <button
          ...
          {tab === "review" && (
            <div className="coverage-panel">
              ...
              <h2>
                {data.unresolved
                  ? `${data.unresolved} shot${data.unresolved === 1 ? " needs" : "s need"} a decision`
                  : "Scene coverage is confirmed"}
              </h2>
              <p>
                Only human-confirmed take ranges enter this reel. AI
                recommendations remain in the shot cockpit until someone
                confirms them.
              </p>
                ...
                {data.findings.length ? (
                  data.findings.map((finding, findingIndex) => {
                    const entry = data.entries.find(
                      (candidate) => candidate.clip_id === finding.clip_id,
                    );
                    return (
                      <button
                        key={`${finding.clip_id}-${findingIndex}`}
                        onClick={() =>
                          entry
                            ? openEntry(entry, finding.start_s)
                            : window.location.assign(
                                `${paths.shot(projectId, sceneId, finding.shot)}?clip=${finding.clip_id}&at=${finding.start_s}`,
                              )
                        }
                      >
                        <span
                          className={`issue-severity ${finding.severity}`}
                        />
                        <span>
                          <b>{finding.code.replaceAll(".", " ")}</b>
                          <small>
                            Shot {finding.shot} · Take {finding.take_no} ·{" "}
                            {clock(finding.start_s)}
                            {entry ? "" : " · open in Shot Review"}
                          </small>
                        </span>
                        <i>›</i>
                      </button>
                  ...
                  <p className="empty-panel">No open findings on this scene.</p>
                )}
              </div>
            </div>
          )}
            ...
            <div className="coverage-panel">
              ...
              {data.notes.length ? (
                data.notes.map((note, noteIndex) => {
                  const entry = data.entries.find(
                    (candidate) => candidate.clip_id === note.clip_id,
                  );
                  return (
                    <button
                      className="coverage-note"
                      key={`${note.clip_id}-${noteIndex}`}
                      onClick={() =>
                        entry
                          ? openEntry(entry, note.at_s)
                          : window.location.assign(
                              `${paths.scene(projectId, sceneId)}?clip=${note.clip_id}&at=${note.at_s}`,
                            )
                      }
                    >
                ...
                <p className="empty-panel">No open notes.</p>
              )}
            </div>
          )}
          {tab === "activity" && (
            <div className="coverage-panel">
              <h2>Scene activity</h2>
              {data.activity.map((row, rowIndex) => (
                <div className="activity-row" key={`${row.at}-${rowIndex}`}>
                  <span className="avatar">
                    {row.actor.slice(0, 1).toUpperCase()}
                  </span>
                  <span>
                    <b>{row.actor.split("@")[0] || "System"}</b>
                    <p>
                      {row.verb.replaceAll("_", " ")} · {row.detail}
                    </p>
                    <small>
                      {row.at ? archiveLocal(row.at) : ""}
                    </small>
                  </span>
                </div>
              ))}
