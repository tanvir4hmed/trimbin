...
export default function SceneCoveragePage({
  ...
  const selectedShots = useMemo(() => data?.timeline.filter((item) => (item.entries?.length ?? 0) > 0 || item.entry) ?? [], [data]);
  const active = entries[index];
  const activeShotIndex = selectedShots.findIndex((item) => (item.entries ?? (item.entry ? [item.entry] : [])).some((entry) => entry.segment_id ? entry.segment_id === active?.segment_id : entry.clip_id === active?.clip_id && entry.start_s === active?.start_s));
  ...
  const moveShot = (direction: number) => {
    const next = selectedShots[Math.max(0, Math.min(selectedShots.length - 1, activeShotIndex + direction))];
    const entry = next?.entries?.[0] ?? next?.entry;
    if (entry) openEntry(entry);
  };

  if (error) return <main className="scene-state"><p className="error">{error}</p><Link href={`${paths.project(projectId)}`}>Back to project</Link></main>;
  if (!data) return <main className="scene-state">Loading coverage…</main>;

  return <main className="coverage-shell">
    <header className="coverage-head"><div className="crumbs"><Link href={`${paths.project(projectId)}`}>Project</Link><span>›</span><b>Scene {sceneId}</b><span className="coverage-mode">Coverage Reel</span></div><div className="coverage-stats"><span>{selectedShots.length}/{data.shots} shots confirmed</span><span>{clock(data.duration_s)}</span><a className="ghost small" href={api.edlUrl(projectId, sceneId, data.export_fps || 24)}>EDL</a><a className="ghost small" href={api.markersUrl(projectId, sceneId, data.export_fps || 24)}>Markers</a></div></header>
